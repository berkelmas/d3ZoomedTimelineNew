...
},{}],"utilities/add-defs.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.rescaleLinearGradientOnZoom = exports.addMainPathLinearDef = void 0;
...
const xscaleGradient = d3.scaleTime().domain(d3.extent([new Date(2020, 0, 1), new Date(2020, 11, 31)])).range([0, 1]);

const addMainPathLinearDef = mainSvg => {
  const mainDef = mainSvg.append("defs");
  const linearGradientDef = mainDef.append("linearGradient").attr("id", "main-path-gradient").attr("y1", 1.5).attr("x2", 880).attr("y2", 1.5).attr("gradientUnits", "userSpaceOnUse");
  linearGradientDef.append("stop").attr("offset", 0).attr("stop-color", "#43a0ee");
  linearGradientDef.append("stop").attr("id", "linear-first-point").attr("offset", xscaleGradient(new Date())).attr("stop-color", "#43a0ee");
  linearGradientDef.append("stop").attr("id", "linear-second-point").attr("offset", xscaleGradient(new Date()) + 0.01).attr("stop-color", "#afafaf");
};

exports.addMainPathLinearDef = addMainPathLinearDef;

const rescaleLinearGradientOnZoom = new_xScale => {
...
exports.rescaleLinearGradientOnZoom = rescaleLinearGradientOnZoom;
},{"d3":"node_modules/d3/index.js"}],"utilities/cluster-builder.js":[function(require,module,exports) {
"use strict";
...
const clusterBuilder = (data, clusterDay) => {
  const clusterData = [];
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = i + 1; j < data.length; j++) {
      if (_dayCountBetweenDates(data[j]["date"], data[i]["date"]) < clusterDay) {
        if (!data[j]["used"]) {
  ...
  for (let i = 0; i < clusterData.length; i++) {
    if ("type" in clusterData[i]) {
      ...
      // }
      const total = clusterData[i]["children"].reduce((prev, curr) => prev + curr["nthDay"], 0); // const middlePoint = total / clusterData[i]["children"].length;
...
},{}],"utilities/globalVals.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.setCurrentScale = exports.setCurrentZoom = exports.setSelectedDate = exports.x_axis = exports.currentScale = exports.xscale = exports.currentZoom = exports.selectedDate = exports.height = exports.width = void 0;
...
let selectedDate = new Date(2020, 1, 1);
exports.selectedDate = selectedDate;
let currentZoom = 1;
exports.currentZoom = currentZoom;
const xscale = d3.scaleTime().domain(d3.extent([new Date(selectedDate.getFullYear(), 0, 1), new Date(selectedDate.getFullYear(), 11, 31)])).range([0, width - 100]);
...
const setSelectedDate = newDate => {
  exports.selectedDate = selectedDate = newDate;
};

exports.setSelectedDate = setSelectedDate;

const setCurrentZoom = newZoom => {
  exports.currentZoom = currentZoom = newZoom;
};

exports.setCurrentZoom = setCurrentZoom;

const setCurrentScale = newScale => {
  exports.currentScale = currentScale = newScale;
};

exports.setCurrentScale = setCurrentScale;
},{"d3":"node_modules/d3/index.js"}],"utilities/styleAxis.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.styleAxis = void 0;

var _addDefs = require("./add-defs");

const styleAxis = (mainSvg, xscale) => {
  mainSvg.selectAll("g .tick").selectAll("line").attr("stroke", "#43A0EE").style("opacity", 0).transition().duration(400).style("opacity", 0.4).attr("y2", 100).attr("y1", -100);
  mainSvg.selectAll("g .tick").append("circle").style("fill", "#43A0EE").attr("r", 0).transition().duration(600).attr("r", 3.5);
  mainSvg.selectAll("g .tick").selectAll("text").style("fill", "#43A0EE").style("font-size", "8px");
  mainSvg.selectAll(".domain").attr("stroke", "url(#main-path-gradient)"); // CIRCLE COLOR TRANSFORMATION

  mainSvg.selectAll("circle").filter(function (d, i) {
    // const todaysExtent = xscale(new Date());
    return d <= new Date() ? false : true; // const transformAtt = this.parentNode.getAttribute("transform");
    // const parentTransformationVal = +transformAtt.match(/[0-9]+[.][0-9]+/)[0];
    // return parentTransformationVal <= todaysExtent ? false : true;
  }).style("fill", "#afafaf"); // TEXT COLOR TRANSFORMATION

  mainSvg.selectAll("g .tick").selectAll("text").filter(function (d, i) {
    return d <= new Date() ? false : true; // const todaysExtent = xscale(new Date());
    // const transformAtt = this.parentNode.getAttribute("transform");
    // const parentTransformationVal = +transformAtt.match(/[0-9]+[.][0-9]+/)[0];
    // return parentTransformationVal <= todaysExtent ? false : true;
  }).style("fill", "#afafaf"); // TICK LINE COLOR TRANSFORMATION

  mainSvg.selectAll("g .tick").selectAll("line").filter(function (d, i) {
    return d <= new Date() ? false : true; // const todaysExtent = xscale(new Date());
    // const transformAtt = this.parentNode.getAttribute("transform");
    // const parentTransformationVal = +transformAtt.match(/[0-9]+[.][0-9]+/)[0];
    // return parentTransformationVal <= todaysExtent ? false : true;
  }).style("stroke", "#afafaf");
  (0, _addDefs.rescaleLinearGradientOnZoom)(xscale);
};

exports.styleAxis = styleAxis;
},{"./add-defs":"utilities/add-defs.js"}],"utilities/handleDateChange.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.handleDateChange = void 0;

var d3 = _interopRequireWildcard(require("d3"));

var _globalVals = require("./globalVals");

var _styleAxis = require("./styleAxis");

var _clusterBuilder = require("./cluster-builder");
...
const handleDateChange = (year, onElementClick) => {
  (0, _globalVals.setSelectedDate)(new Date(year, 0, 1));

  _globalVals.xscale.domain(d3.extent([new Date(year, 0, 1), new Date(year, 11, 31)]));

  const svg = d3.select("body").transition();

  if (_globalVals.currentScale) {
    const newXScale = _globalVals.currentScale.rescaleX(_globalVals.xscale);

    svg.select(".x.axis").call(d3.axisBottom().scale(newXScale));
    d3.selectAll("#data-container-rect").remove();
    const newClusterData = (0, _clusterBuilder.clusterBuilder)(_mockData.notificationsData, _globalVals.currentScale ? 40 / _globalVals.currentScale.k : 40);
    const newRectEls = d3.select("#js-svg").selectAll(".rect").data(newClusterData).enter().append("g").attr("id", "data-container-rect"); // RENDER NORMAL DATA

    newRectEls.append("rect").filter(d => d["type"] !== "cluster").attr("x", d => newXScale(d.date)).attr("width", 55).attr("height", 16).attr("y", 60).attr("r", 5).attr("rx", 2).style("fill", "#43A0EE").style("cursor", "pointer").on("click", d => onElementClick(d)); // RENDER CLUSTERS

    newRectEls.append("rect").filter(d => d["type"] === "cluster").attr("x", d => newXScale(d.date)).attr("width", 27).attr("height", 27).attr("y", 48).attr("r", 5).attr("rx", 2).style("fill", "#F5B640"); // ADD TEXT TO NORMAL DATA

    newRectEls.append("text").filter(d => d["type"] !== "cluster").attr("x", function (d, i) {
      return newXScale(d.date) + 10;
    }).attr("y", 71).attr("id", "item-id-text").style("font-size", "9px").style("fill", "white").text(d => d.id).style("cursor", "pointer").on("click", d => onElementClick(d)); // ADD TEXT TO CLUSTERS

    newRectEls.append("text").filter(d => d["type"] === "cluster").attr("x", function (d, i) {
      return newXScale(d.date) + 10;
    }).attr("y", 66).attr("id", "item-id-text").style("font-size", "14px").style("fill", "white").text(d => d.children.length);
    (0, _styleAxis.styleAxis)(d3.select("#js-svg"), newXScale);
    d3.select("#js-svg").selectAll("#todays-line").attr("x", d => newXScale(d) + 44);
  } else {
    svg.select(".x.axis").call(d3.axisBottom().scale(_globalVals.xscale));
    d3.selectAll("#data-container-rect").remove();
    const newClusterData = (0, _clusterBuilder.clusterBuilder)(_mockData.notificationsData, _globalVals.currentScale ? 40 / _globalVals.currentScale.k : 40);
    const newRectEls = d3.select("#js-svg").selectAll(".rect").data(newClusterData).enter().append("g").attr("id", "data-container-rect"); // RENDER NORMAL DATA

    newRectEls.append("rect").filter(d => d["type"] !== "cluster").attr("x", d => (0, _globalVals.xscale)(d.date)).attr("width", 55).attr("height", 16).attr("y", 60).attr("r", 5).attr("rx", 2).style("fill", "#43A0EE").style("cursor", "pointer").on("click", d => onElementClick(d)); // RENDER CLUSTERS

    newRectEls.append("rect").filter(d => d["type"] === "cluster").attr("x", d => (0, _globalVals.xscale)(d.date)).attr("width", 27).attr("height", 27).attr("y", 48).attr("r", 5).attr("rx", 2).style("fill", "#F5B640"); // ADD TEXT TO NORMAL DATA

    newRectEls.append("text").filter(d => d["type"] !== "cluster").attr("x", function (d, i) {
      return (0, _globalVals.xscale)(d.date) + 10;
    }).attr("y", 71).attr("id", "item-id-text").style("font-size", "9px").style("fill", "white").text(d => d.id).style("cursor", "pointer").on("click", d => onElementClick(d)); // ADD TEXT TO CLUSTERS

    newRectEls.append("text").filter(d => d["type"] === "cluster").attr("x", function (d, i) {
      return (0, _globalVals.xscale)(d.date) + 10;
    }).attr("y", 66).attr("id", "item-id-text").style("font-size", "14px").style("fill", "white").text(d => d.children.length);
    (0, _styleAxis.styleAxis)(d3.select("#js-svg"), _globalVals.xscale);
    d3.select("#js-svg").selectAll("#todays-line").attr("x", d => (0, _globalVals.xscale)(d) + 44);
  }
};

exports.handleDateChange = handleDateChange;
},{"d3":"node_modules/d3/index.js","./globalVals":"utilities/globalVals.js","./styleAxis":"utilities/styleAxis.js","./cluster-builder":"utilities/cluster-builder.js","../data/mock-data":"data/mock-data.js"}],"main.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.zoomOnElement = exports.createTimeline = void 0;

var d3 = _interopRequireWildcard(require("d3"));

var _mockData = require("./data/mock-data");

var _addDefs = require("./utilities/add-defs");

var _clusterBuilder = require("./utilities/cluster-builder");

var _handleDateChange = require("./utilities/handleDateChange");

var _globalVals = require("./utilities/globalVals");

var _styleAxis = require("./utilities/styleAxis");
...
const createTimeline = (selector, onZoom, onElementClick) => {
  const clusteredData = (0, _clusterBuilder.clusterBuilder)(_mockData.notificationsData, 40);
  const svgContainer = d3.select(selector).append("svg").attr("viewBox", `0 0 ${_globalVals.width} ${200}`).attr("id", "js-svg");
  mainSvg = d3.select("#js-svg");
  (0, _addDefs.addMainPathLinearDef)(mainSvg);
  const xAxisTranslate = _globalVals.height / 3.5;
  const gX = mainSvg.append("g").classed("x axis", true).attr("transform", "translate(50, " + xAxisTranslate + ")").call(_globalVals.x_axis);
  zoom = d3.zoom().translateExtent([[0, 0], [_globalVals.width, _globalVals.height]]).scaleExtent([1, 50]).on("zoom", () => {
    const pointToDateScale = d3.scaleLinear().domain([0, 900]).range([new Date(_globalVals.selectedDate.getFullYear(), 0, 1), new Date(_globalVals.selectedDate.getFullYear(), 11, 31)]); // zoom level is between 1 - 50;

    (0, _globalVals.setCurrentZoom)(d3.event.transform.k); // AXIS STARTING DATE

    const axisStartDate = pointToDateScale(-d3.event.transform.x / _globalVals.currentZoom);
    const axisEndDate = pointToDateScale(-d3.event.transform.x / _globalVals.currentZoom + 900 / _globalVals.currentZoom);
    onZoom(axisStartDate, axisEndDate);
    const newClusterData = (0, _clusterBuilder.clusterBuilder)(_mockData.notificationsData, 40 / _globalVals.currentZoom);
    const new_xScale = d3.event.transform.rescaleX(_globalVals.xscale);
    (0, _globalVals.setCurrentScale)(d3.event.transform);
    d3.selectAll("#data-container-rect").remove();
    const newRectEls = mainSvg.selectAll(".rect").data(newClusterData).enter().append("g").attr("id", "data-container-rect"); // RENDER NORMAL DATA

    newRectEls.append("rect").filter(d => d["type"] !== "cluster").attr("x", d => {
      ...
      return pos;
    }).attr("width", 55).attr("height", 16).attr("y", 60).attr("r", 5).attr("rx", 2).style("fill", "#43A0EE").style("cursor", "pointer").on("click", d => onElementClick(d)); // RENDER CLUSTERS

    newRectEls.append("rect").filter(d => d["type"] === "cluster").attr("x", d => new_xScale(d.date)).attr("width", 27).attr("height", 27).attr("y", 48).attr("r", 5).attr("rx", 2).style("fill", "#F5B640"); // ADD TEXT TO NORMAL DATA
    ...
    gX.call(_globalVals.x_axis.scale(new_xScale));
    mainSvg.selectAll("rect").attr("x", d => new_xScale(d.date));
    mainSvg.selectAll("#item-id-text").attr("x", d => new_xScale(d.date) + 10);
    mainSvg.selectAll("g .tick").selectAll("line").style("opacity", 0.4).attr("stroke", "#43A0EE").attr("y2", 100).attr("y1", -100);
    mainSvg.selectAll("g .tick").append("circle").style("fill", "#43A0EE").attr("r", 3.5);
    mainSvg.selectAll("g .tick").selectAll("text").style("fill", "#43A0EE").style("font-size", "8px");
    mainSvg.selectAll("#todays-line").attr("x", d => new_xScale(d) + 44); // TICK CIRCLE COLOR TRANSFORMATION

    mainSvg.selectAll("circle").filter(function (d, i) {
      return d <= new Date() ? false : true;
    }).style("fill", "#afafaf"); // TICK TEXT COLOR TRANSFORMATION
    ...
    (0, _addDefs.rescaleLinearGradientOnZoom)(new_xScale);
    console.log(processedData);
  });
  mainSvg.call(zoom);
  const todaysEl = mainSvg.selectAll(".today-line").data([new Date()]).enter().append("g");
  todaysEl.append("rect").attr("id", "todays-line").attr("x", d => (0, _globalVals.xscale)(d) + 44).attr("width", 2).attr("height", 188).attr("y", 0).attr("r", 5).attr("rx", 2).style("fill", "#43A0EE");
  const rectEls = mainSvg.selectAll(".rect").data(clusteredData).enter().append("g").attr("id", "data-container-rect"); // RENDER NORMAL DATA

  rectEls.append("rect").filter(d => d["type"] !== "cluster").attr("x", d => (0, _globalVals.xscale)(d.date)).attr("width", 55).attr("height", 16).attr("y", 60).attr("r", 5).attr("rx", 2).style("cursor", "pointer").on("click", d => onElementClick(d)).style("fill", "#43A0EE"); // RENDER CLUSTERS

  rectEls.append("rect").filter(d => d["type"] === "cluster").attr("x", d => (0, _globalVals.xscale)(d.date)).attr("width", 27).attr("height", 27).attr("y", 48).attr("r", 5).attr("rx", 2).style("fill", "#F5B640"); // ADD TEXT TO NORMAL DATA

  rectEls.append("text").filter(d => d["type"] !== "cluster").attr("x", function (d, i) {
    return (0, _globalVals.xscale)(d.date) + 10;
  }).attr("y", 71).attr("id", "item-id-text").style("font-size", "9px").style("fill", "white").text(d => d.id).style("cursor", "pointer").on("click", d => onElementClick(d)); // ADD TEXT TO CLUSTERS
  ...
  (0, _styleAxis.styleAxis)(mainSvg, _globalVals.xscale); // setTimeout(() => {
  //   zoom.scaleBy(mainSvg, 8.597564862430055);
  //   zoom.translateTo(mainSvg, 176.5835428082192);
  // }, 2000);
  // GET INITIAL LOCATIONS OF ALL DATA

  initialLocations = _mockData.notificationsData.map(item => ({ ...item,
    xPos: (0, _globalVals.xscale)(item.date)
  }));
};

exports.createTimeline = createTimeline;

const zoomOnElement = id => {
  zoom.scaleBy(mainSvg, 8.5);
  zoom.translateTo(mainSvg, initialLocations.find(item => item.id === id).xPos);
};

exports.zoomOnElement = zoomOnElement;
},{"d3":"node_modules/d3/index.js","./data/mock-data":"data/mock-data.js","./utilities/add-defs":"utilities/add-defs.js","./utilities/cluster-builder":"utilities/cluster-builder.js","./utilities/handleDateChange":"utilities/handleDateChange.js","./utilities/globalVals":"utilities/globalVals.js","./utilities/styleAxis":"utilities/styleAxis.js"}],"index.js":[function(require,module,exports) {
"use strict";

var _main = require("./main");

(0, _main.createTimeline)("#container", () => console.log("deneme"), d => (0, _main.zoomOnElement)(d.id));
document.getElementById("zoom-button").addEventListener("click", _ => {
  const val = document.getElementById("id-input").value;
  (0, _main.zoomOnElement)(val);
});
...
//# sourceMappingURL=/d3Timeline.e31bb0bc.js.map